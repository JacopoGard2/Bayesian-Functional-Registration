...
calcola_norme_gruppo = function(v,w){
  errore_true_1 = rep(0,n_groups)
  errore_true_norm_1 = rep(0,n_groups)
  errore_true_2 = rep(0,n_groups)
  errore_true_norm_2 = rep(0,n_groups)
  errore_true_MSE = rep(0,n_groups)
  errore_true_RMSE = rep(0,n_groups)
  errore_true_inf = rep(0,n_groups)
  errore_true_norm_inf = rep(0,n_groups)
  
  for( g in 1:n_groups){
    # Norma 1
    errore_true_1[g] = sum(abs(v[g,] - w[[g]]))
    errore_true_norm_1[g] = errore_true_1[g] / sum(abs(v[g,] ))
    
    # Norma 2 
    errore_true_2[g] = sqrt(sum((v[g,] - w[[g]])^2))
    errore_true_norm_2[g] = errore_true_2[g] / sqrt(sum(v[g,]^2 ))
    
    # MSE
    errore_true_MSE[g] = mean((v[g,] - w[[g]])^2)
    errore_true_RMSE[g] = sqrt(errore_true_MSE[g])
    
    # Norma inf 
    errore_true_inf[g] = max(abs(v[g,] - w[[g]]))
    errore_true_norm_inf[g] = errore_true_inf[g] / max(abs(v[g,]))
  }
  err_1 = mean(errore_true_1)
  err_2 = mean(errore_true_2)
  err_MSE = mean(errore_true_MSE)
  err_RMSE = mean(errore_true_RMSE)
  err_inf = mean(errore_true_inf)
  err_norm_1 = mean(errore_true_norm_1)
  err_norm_2 = mean(errore_true_norm_2)
  err_norm_inf = mean(errore_true_norm_inf)
  ...
  return(list(err=list(norm_1=err_1,norm_2=err_2,norm_inf=err_inf,MSE=err_MSE,RMSE= err_RMSE), 
              ...
              gruppo=list(err=list(norm_1=errore_true_1,norm_2=errore_true_2,norm_inf=errore_true_inf,MSE=errore_true_MSE, RMSE = errore_true_RMSE),
                          err_rel = list(norm_1_rel = errore_true_norm_1,norm_2_rel = errore_true_norm_2,norm_inf_rel = errore_true_norm_inf))
  ))
}

calcola_norme_gruppo_ind = function(v,w){
  ...
  for( g in 1:n_groups){
    # Norma 1
    err_true_1[g] = mean(sapply(1:n_per_group[g], function(i) sum(abs(v[[g]][i, ] - w[[g]][[i]])) ) )
    err_true_norm_1[g] = mean(sapply(1:n_per_group[g], function(i) sum(abs(v[[g]][i, ] - w[[g]][[i]]))/sum(abs(v[[g]][i, ])) ))
    
    # Norma 2 
    err_true_2[g] = mean(sapply(1:n_per_group[g], function(i) sqrt(sum((v[[g]][i, ] - w[[g]][[i]])^2) ) ) )
    err_true_norm_2[g] = mean(sapply(1:n_per_group[g], function(i) sqrt(sum((v[[g]][i, ] - w[[g]][[i]])^2)) / sqrt(sum(v[[g]][i, ]^2)) ) ) 
    
    # MSE
    err_true_MSE[g] = mean(sapply(1:n_per_group[g], function(i) mean((v[[g]][i, ] - w[[g]][[i]])^2) ) ) 
    err_true_RMSE[g] = sqrt(err_true_MSE[g])
    
    # Norma inf 
    err_true_inf[g] = mean(sapply(1:n_per_group[g], function(i) max(abs(v[[g]][i, ] - w[[g]][[i]])) ) ) 
    err_true_norm_inf[g] = mean(sapply(1:n_per_group[g], function(i) max(abs(v[[g]][i, ] - w[[g]][[i]])) / max(abs(v[[g]][i, ]) ) ) )
  }
...
calcola_norme_gruppo_curve = function(v,w){
  ...
  for( g in 1:n_groups){
    # Norma 1
    err_true_1[g] = mean(sapply(1:n_per_group[g], function(i) sum(abs(v[[g]][1:N_OBS,i ] - w[[g]][[i]])) ) )
    err_true_norm_1[g] = mean(sapply(1:n_per_group[g], function(i) sum(abs(v[[g]][1:N_OBS,i ] - w[[g]][[i]]))/sum(abs(v[[g]][i, ])) ))
    
    # Norma 2 
    err_true_2[g] = mean(sapply(1:n_per_group[g], function(i) sqrt(sum((v[[g]][1:N_OBS, i] - w[[g]][[i]])^2) ) ) )
    err_true_norm_2[g] = mean(sapply(1:n_per_group[g], function(i) sqrt(sum((v[[g]][1:N_OBS,i ] - w[[g]][[i]])^2)) / sqrt(sum(v[[g]][i, ]^2)) ) ) 
    
    # MSE
    err_true_MSE[g] = mean(sapply(1:n_per_group[g], function(i) mean((v[[g]][1:N_OBS, i] - w[[g]][[i]])^2) ) ) 
    err_true_RMSE[g] = sqrt(err_true_MSE[g])
    
    # Norma inf 
    err_true_inf[g] = mean(sapply(1:n_per_group[g], function(i) max(abs(v[[g]][1:N_OBS,i ] - w[[g]][[i]])) ) ) 
    err_true_norm_inf[g] = mean(sapply(1:n_per_group[g], function(i) max(abs(v[[g]][1:N_OBS,i ] - w[[g]][[i]])) / max(abs(v[[g]][i, ]) ) ) )
  }
  err_1 = mean(err_true_1)
  err_2 = mean(err_true_2)
  err_MSE = mean(err_true_MSE)
...
calcola_errori = function(current_data,result){
  #-- PARAMS --#
  # Beta 
  err_beta = calcola_norme_gruppo(current_data$params$beta,result$post$beta_group)
  
  # Lambda 
  err_lambda = list(err=abs(current_data$params$lambda-result$post$lambda),err_rel = abs(current_data$params$lambda-result$post$lambda) / current_data$params$lambda )
  
  # Gamma 
  err_gamma = calcola_norme_gruppo_ind(current_data$params$gamma,result$post$gamma_group)
  
  # Sigma gamma 
  err_sigma_gamma = list(err = abs(current_data$params$sigma_gamma-result$post$sigma_gamma),err_rel=abs(current_data$params$sigma_gamma-result$post$sigma_gamma) / current_data$params$sigma_gamma )
  
  # Sigma epsilon 
  err_sigma_eps = list(err=abs(current_data$params$sigma_eps-result$post$sigma_eps),err_rel= abs(current_data$params$sigma_eps-result$post$sigma_eps)/ current_data$params$sigma_eps)
  
  # Csi
  err_csi = calcola_norme_gruppo_ind(current_data$params$csi,result$post$csi_group)
  
  # Phi 
  err_phi = calcola_norme_gruppo_ind(current_data$params$phi,result$post$phi_group)
  
  #-- CURVES --#
  # Misaligned
  err_y_mis = calcola_norme_gruppo_curve(current_data$y$y_mis,result$post$y)
  
  # SMOOTED Misaligned
  err_y_mis_smoot = calcola_norme_gruppo_curve(current_data$y$y_mis_noise_free,result$post$y)
  
  # Allineate 
  err_y_al = calcola_norme_gruppo_curve(current_data$y$y_al,result$y_star)
  
  # Allineate SMOOT 
  err_y_al_smoot = calcola_norme_gruppo_curve(current_data$y$y_al_noise_free, result$y_star_smoot)
  
  return(list( params = list(
    beta = err_beta, lambda = err_lambda, gamma = err_gamma, sigma_gamma = err_sigma_gamma, 
    sigma_eps = err_sigma_eps,csi = err_csi, phi = err_phi),
...
errors = calcola_errori(dati_sim,result)

errors$curves$y_al$err$RMSE

save(errors,file="errore_sim1_OURmodel_seed100.RData")
...
y_to_use = dati_sim$y$y_to_use

#-- Aligned curves group 2 --# 
x11()
par(mfrow=c(1,2))
plot(seq(0,1, len=n_obs[[2]][1]),dati_sim$y$y_to_use[1:n_obs[[2]][1],10], type='l', main='',
     col=reds[1], xlab='time', ylab='y',ylim=ylim_plot)
for(i in 2:n_per_group[2]){
  points(seq(0,1, len=n_obs[[2]][i]),dati_sim$y$y_to_use[1:n_obs[[2]][i],10+i], type='l', col=reds[i])
}
...
#-- Aligned smooted curves surgery --# 
x11()
par(mfrow=c(1,2))
plot(seq(0,1, len=n_obs[[2]][1]),y_to_use[1:n_obs[[2]][1],10], type='l', main='',
     col=reds[1], xlab='', ylab='',ylim=ylim_plot)
for(i in 2:n_per_group[2]){
  points(seq(0,1, len=n_obs[[2]][i]),y_to_use[1:n_obs[[2]][i],10+i], type='l', col=reds[i])
}
points(seq(0,1, len=100),BM%*%result$post$beta_group[[2]],col='coral1',lwd=5,type='l')
plot(seq(0,1, len=n_obs[[2]][1]),result$y_star_smoot[[2]][[1]], type='l', main='',
     col=reds[1], xlab='', ylab='',ylim=ylim_plot)
for(i in 2:n_per_group[2]){
  points(seq(0,1, len=n_obs[[2]][i]),result$y_star_smoot[[2]][[i]], type='l', col=reds[i])
}
points(seq(0,1, len=100),BM%*%result$post$beta_group[[2]],col='coral1',lwd=5,type='l')
