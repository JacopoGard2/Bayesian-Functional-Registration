...
JARA_warping_GROUP = function(y,n_groups = 3, n_per_group = c(32,32,36), nburn=100,niter=100,
                              ...
                              INIT_WARP = F, phi_init, SOMMA_csi
)
{
    ...
    for(g in 1:n_groups){
      ...
      for(i in 1:n_per_group[g]){
        ...
        if(INIT_WARP){
          ...
          for(j in 1:q){
            if(j==1){
              csi_save_ind[1,j] = SOMMA_csi * phi_init[j]
              x_save_ind[1,j] = 0
            }else{
              csi_save_ind[1,j] = SOMMA_csi * (phi_init[j]-phi_init[j-1])
...
load("control_group.RData")
y_control = y
load("surgery_group.RData")
y_surgery = y 
load("physiotherapy_group.RData")
y_physio = y
rm(y)

add_na_rows <- function(mat, target_rows) {
  current_rows <- nrow(mat)  # Numero di righe attuali
  if (current_rows < target_rows) {
    rows_to_add <- target_rows - current_rows  # Quante righe aggiungere
    # Aggiungi righe con NA per tutte le colonne
    matrix_to_add = matrix(NA, nrow = rows_to_add, ncol = ncol(mat))
    colnames(matrix_to_add) = colnames(mat)
    mat <- rbind(mat, matrix_to_add )
  }
  return(mat)
}

y_control = add_na_rows(y_control,591)
y_surgery = add_na_rows(y_surgery,591)
y_physio = add_na_rows(y_physio,591)


y_fix = cbind(y_control,y_surgery,y_physio)

y = y_fix 

n_groups = 3
n_per_group = c(32,32,36)
...
result = JARA_warping_GROUP(y, n_groups = n_groups, n_per_group = n_per_group,
                            ...
                            INIT_WARP = INIT_WARP, phi_init = phi_init, SOMMA_csi = SOMMA_csi ) 
